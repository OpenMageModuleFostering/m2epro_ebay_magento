...
class Ess_M2ePro_Adminhtml_Common_Play_ListingController
    extends Ess_M2ePro_Controller_Adminhtml_Common_MainController
{
    ...
    protected function _initAction()
    {
        $this->loadLayout()
            ...
            ->_title(Mage::helper('M2ePro')->__('Play.com Listings'));
        ...
        $this->getLayout()->getBlock('head')
            ...
            ->addJs('M2ePro/Common/Play/Listing/GridHandler.js')

            ->addJs('M2ePro/ActionHandler.js')
            ->addJs('M2ePro/Listing/ActionHandler.js')
            ->addJs('M2ePro/Listing/MovingHandler.js')
            ->addJs('M2ePro/Common/Play/Listing/ProductSearchHandler.js')

            ->addJs('M2ePro/TemplateHandler.js')
            ->addJs('M2ePro/Common/Listing/SettingsHandler.js')
            ->addJs('M2ePro/Common/Play/Listing/ChannelSettingsHandler.js')
            ->addJs('M2ePro/Common/Play/Listing/ProductsFilterHandler.js')
    ...
    {
        $block = $this->loadLayout()->getLayout()->createBlock('M2ePro/adminhtml_common_play_listing_grid');
    ...
    {
        $this->_initAction()
            ->_addContent($this->getLayout()->createBlock('M2ePro/adminhtml_common_play_listing_search'))
            ->renderLayout();
    }

    public function searchGridAction()
    {
        $block = $this->loadLayout()->getLayout()->createBlock('M2ePro/adminhtml_common_play_listing_search_grid');
        $this->getResponse()->setBody($block->toHtml());
    }
    ...
    public function viewAction()
    {
        ...
        if ($model->isLockedObject('products_in_action')) {
            $this->_getSession()->addNotice(
                Mage::helper('M2ePro')->__('Some Play.com request(s) are being processed now.')
            );
        }
        //----------------------------
        ...
        // Set rule model
        // ---------------------------
        $this->setRuleData('play_rule_listing_view');
        // ---------------------------

        $this->_initAction()
            ->_addContent($this->getLayout()->createBlock('M2ePro/adminhtml_common_play_listing_view'))
            ->renderLayout();
    }
    ...
    public function viewGridAction()
    {
        ...
        $response = $this->loadLayout()->getLayout()
            ->createBlock('M2ePro/adminhtml_common_play_listing_view_grid')->toHtml();
    ...
    {
        $id = $this->getRequest()->getParam('id');
        $listing = Mage::helper('M2ePro/Component_Play')->getCachedObject('Listing',$id);
        ...
        $this->_initAction();
        $this->_addContent($this->getLayout()->createBlock('M2ePro/adminhtml_common_play_listing_edit'));
        $this->renderLayout();
    }
    ...
    public function saveAction()
    {
        ...
        $id = $this->getRequest()->getParam('id');
        $model = Mage::helper('M2ePro/Component_Play')->getModel('Listing')->load($id);
    ...
    protected function processConnector($action, array $params = array())
    {
        ...
        $listingProductObject = Mage::helper('M2ePro/Component_Play')
    ...
    public function getSuggestedPlayIDGridAction()
    {
        ...
        if (!empty($searchSettingsData['data'])) {
            ...
            $response = $this->loadLayout()->getLayout()
                ->createBlock('M2ePro/adminhtml_common_play_listing_productSearch_grid')->toHtml();
        } else {
    ...
    public function searchPlayIDManualAction()
    {
        $productId = $this->getRequest()->getParam('product_id');
        ...
        $tempCollection = Mage::helper('M2ePro/Component_Play')->getCollection('Listing_Product');
    ...
    public function mapToPlayIDAction()
    {
        $productId = $this->getRequest()->getParam('product_id');
        ...
        if ($listingProduct->isNotListed() &&
            ...
            $listingProduct->setData(
                'general_id_type', Ess_M2ePro_Model_Play_Listing::GENERAL_ID_MODE_GENERAL_ID
            );
    ...
    protected function setRuleData($prefix)
    {
        ...
        $ruleModel = Mage::getModel('M2ePro/Play_Magento_Product_Rule')->setData(
            array(
                'prefix' => $prefix,
                'store_id' => $storeId,
            )
    ...
    protected function getHideProductsInOtherListingsPrefix()
    {
        ...
        $prefix = 'play_hide_products_others_listings_';
