EbayListingTemplateSwitcherHandler = Class.create(CommonHandler, {
    ...
    change: function()
    {
        var templateNick = EbayListingTemplateSwitcherHandlerObj.getSwitcherNickByElementId(this.id);
        var templateMode = EbayListingTemplateSwitcherHandlerObj.getSwitcherValueMode(templateNick);

        EbayListingTemplateSwitcherHandlerObj.clearMessages(templateNick);
        ...
        switch (templateMode) {
            ...
            case M2ePro.php.constant('Ess_M2ePro_Model_Ebay_Template_Manager::MODE_CUSTOM'):
                EbayListingTemplateSwitcherHandlerObj.reloadContent(templateNick);
                break;

            case M2ePro.php.constant('Ess_M2ePro_Model_Ebay_Template_Manager::MODE_TEMPLATE'):
                EbayListingTemplateSwitcherHandlerObj.clearContent(templateNick);
                EbayListingTemplateSwitcherHandlerObj.checkMessages(templateNick);
                break;
        }

        EbayListingTemplateSwitcherHandlerObj.hideEmptyOption(EbayListingTemplateSwitcherHandlerObj.getSwitcher(templateNick));

        if (!EbayListingTemplateSwitcherHandlerObj.isSwitcherValueModeCustom(templateNick)) {
            EbayListingTemplateSwitcherHandlerObj.updateButtonsVisibility(templateNick);
            EbayListingTemplateSwitcherHandlerObj.updateEditVisibility(templateNick);
            EbayListingTemplateSwitcherHandlerObj.updateTemplateLabelVisibility(templateNick);
        }
    },
    ...
    saveSwitchers: function(callback)
    {
        ...
        if (EbayListingTemplateSwitcherHandlerObj.isExistSynchronizationTab()) {
            EbayTemplateSynchronizationHandlerObj.checkVirtualTabValidation();
        }

        if (!validationResult) {
            EbayListingTemplateSwitcherHandlerObj.scrollToFirstFailedElement();
            return;
        }
        ...
        $('edit_form').request({
            ...
            onSuccess: function(transport) {
                ...
                response.each(function(template) {
                    EbayListingTemplateSwitcherHandlerObj.afterCustomSaveAsTemplate(template.nick, template.id, template.title);
                });
    ...
    afterCustomSaveAsTemplate: function(templateNick, templateId, templateTitle)
    {
        ...
        var switcher = EbayListingTemplateSwitcherHandlerObj.getSwitcher(templateNick);
    ...
    customizeTemplate: function(templateNick)
    {
        ...
        this.reloadContent(templateNick, function() {
            $$('input[name="'+templateNick+'[id]"]')[0].value = EbayListingTemplateSwitcherHandlerObj.getSwitcherValueId(templateNick);
